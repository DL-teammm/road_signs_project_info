...
При оценке эффективности такого подхода, последовательной работы детекционной и классификационной моделей, было зафиксирована достаточно низкая метрика mAP50, около 0.4, подсчёт которой сохранён в ноутбуке [det_plus_class_mAP50](https://github.com/DL-teammm/road_signs_recognition/blob/main/notebooks/det_plus_class_mAP50.ipynb). Однако, при оценке скорости работы совмещённых моделей, тестирования на GPU T4 не превышали 0.06 секунд на обработку одного кадра, тестирования скорости работы моделей на CPU показывают около 0.3 секунды на кадр. Таким образом, мы действительно можем рассмотреть данный вариант для дальнейшей обработки кадров в реальном времени.
...
Финальная версия пайплайна с использованием только детектора yolov8m на 146 классах дорожных знаков реализована в ```chain.py``` внутри веб-сервиса: [chain](https://github.com/DL-teammm/web_service/blob/main/chain.py).

Веса лучшей модели, которые нужны для запуска приложения, можно загрузить из [этой](https://drive.google.com/drive/folders/1JM2FCMNGXQiQFqoPMV6aMzxMmzYPl9ri?usp=sharing) папки на диске.

**Производительность**: на gpu T4 FPS инференса детекционной модели в среднем составляет около 65, что соответствует real-time.

**Качественная оценка**:
- большую часть дорожных знаков находим и правильно определяем класс;
- хорошо справляемся с разными условиями съёмки: и в тёмное, и в светлое время суток;
- на тестовых видео встречались типы знаков, которых не было в датасете: такие знаки модель определяет неправильно (например, вместо знака 5.2, которого не было в датасете, распознаёт знак 5.6);
- не детектим знаки расположенные далеко от камеры, при приближении камеры начинаем правильно распознавать;
- хуже работаем, когда на видео идёт снег.

С результатами тестирования подробнее можно ознакомиться здесь: [tests_run_on_gpu](https://drive.google.com/drive/folders/1hWNshUzVXTA5ovKDhEN_gHDfLgOVub16?usp=sharing) -- сохранены видео с отрисовкой результатов и логи запуска со скоростью работы пайплайна на gpu T4.
